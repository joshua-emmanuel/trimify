...
export default function EditShortLinkForm({
  closeForm,
  link,
  refetchLinks,
}: Props) {
  ...
  const editShortLink = async ({
    id,
    title,
    originalUrl,
    shortUrl,
  }: EditShortLinkTypes): Promise<void> => {
    try {
      setLoading(true);
      const response = await fetch('/api/shorten', {
        ...
        body: JSON.stringify({
          ...
          title,
          originalUrl,
          shortUrl: shortUrl !== link.short_url ? shortUrl : undefined,
        }),
      });
  ...
  const onSubmit = (data: IFormInput) => {
    ...
    editShortLink({ id, title, originalUrl, shortUrl });
  };
